...
O FTSP é um protocolo de sincronização \textit{ad-hoc}, multi-saltos para redes sensores que consegue um bom nível de precisão na faixa de nanosegundos. A alta acurácia é alcançada eliminando o tempo de acesso ao meio através de mensagens com \textit{timestamp} nas camadas mais baixas da pilha de rádio e compensação do escorregamento dos relógios usando regressão linear.
%O protocolo FTSP, bem conhecido por conseguir precisão de nanosegundos com baixa sobrecarga. 

Contudo, o FTSP não é aplicado para todos os tipos de dispositivos, ele depende do recurso de MAC \textit{timestamp} que só está disponível em equipamentos com \textit{chips} de rádio mais sofisticados.
...
Em nossos testes com \textit{motes} reais, os resultados indicaram uma média de erro de sincronização de 1.508$\mu{s}$ por salto, enquanto é adicionada uma mensagem de correção a mais no processo.
