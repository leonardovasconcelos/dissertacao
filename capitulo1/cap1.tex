...
Em sistemas distribuídos, cada nó tem seu próprio relógio e sua própria noção de tempo. No entanto, uma escala de tempo comum entre nós sensores é importante identificar relações causais entre eventos no mundo físico, para apoiar a eliminação de dados redundantes de sensores, e para facilitar a operação da rede de sensores. Uma vez que cada nó em uma rede de sensores opera de forma independente e conta com o seu próprio relógio, as leituras do relógio de diferentes nós de sensores também será diferente. Além destas diferenças aleatórias (phaseshifts), a diferença entre os relógios de sensores diferentes vai aumentar ainda mais devido às taxas de escorregamento de diferentes osciladores. Portanto, o tempo (ou relógio) de sincronização é necessário para assegurar que os tempos de detecção podem ser comparados de uma forma significativa. 

Embora as técnicas de Sincronização de tempo para redes com fios receberam uma quantidade significativa de atenção, estas técnicas não são adequadas para sensores sem fios, devido aos desafios colocados pelos ambientes sensores sem fio. Estes desafios incluem a potencialmente grande escala da redes de sensores, a necessidade de auto-configuração e robustez, o potencial de mobilidade sensor, e a necessidade de conservação de energia. 

Sincronização de tempo é um serviço necessário para muitas aplicações e serviços em sistemas distribuídos em geral. Numerosos protocolos de sincronização de tempo têm sido propostos para ambos os sistemas com e sem fio, por exemplo, o Network Time Protocol (NTP) \cite{mills1991} é uma abordagem de sincronização escalável, robusto e auto-configurável amplamente implantado. Especialmente em combinação com o Sistema de Posicionamento Global (GPS), tem sido demonstrado para alcançar a precisão da ordem de alguns microssegundos. No entanto, abordagens como NTP não são adequados para RSSFs devido às características e restrições exclusivas dessas redes.
