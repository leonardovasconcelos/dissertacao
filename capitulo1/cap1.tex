...
Sincronização unidirecional ou \textit{one-way} é a forma mais elementar de sincronização \textit{pair-wise}, temos a Figura \ref{fig:pairwise1} que demonstra o seu funcionamento, nela observamos que o nó $i$ no tempo $t_1$ envia seu \textit{timestamp} para $j$, quando o nó $j$ recebe a mensagem ele lê seu relógio local que esta marcando o tempo $t_2$, assim o valor de $t_2$ pode ser calculado como na equação \ref{eq:tempo_t2}:

\begin{equation} \label{eq:tempo_t2}
  t_2 = t_1 + tp + \theta
\end{equation}

Onde $tp$ é o tempo de propagação, este tempo é muito pequeno podendo ser ignorado. O $\theta$ é a diferença entre os tempos de $i$ e $j$, ele indica o \textit{offset} dos relógios. Com essas informações o nó $j$ pode calcular o \textit{offset} e ajustar seu relógio com base no tempo de $i$, podemos calculá-lo com a Equação \ref{eq:tempo_teta}.
...
Sincronização bidirecional ou \textit{tow-way} usa duas mensagens de sincronização. A Figura \ref{fig:pairwise2} mostra o comportamento desta técnica, o nó $i$ envia 
