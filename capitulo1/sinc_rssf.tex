...
Os avanços recentes nos sistemas integrados nos permitem desenvolver dispositivos sensores com baixos recursos de energia, rádio e processamento. Considerando que uma rede de sensores consista em um bom número de sensores, o valor de um só nó afeta a implantação da rede toda. Existe uma tendência defendida por pesquisadores de que com a grande quantidade de sensores aplicados hoje em dia, a produção em larga escala faça com que seu valor possa chegar a ser menor que \$1 dólar \cite{cuevas2007, wang2007topology} por unidade. 

De maneira geral, os \textit{motes} são fabricados com materiais de baixo custo e as vezes de menor qualidade, por exemplo, um cristal de quartzo de baixa qualidade pode ser um dos fatores que tornam o relógios mais imprecisos. A Figura \ref{fig:overhead} apresenta o \textit{tradeoff} entre o  preço e a qualidade. Observe que o preço de produção do dispositivo determina a precisão do seu relógio, assim, quanto menos preciso o relógio é maior compensação será necessária para corrigir seu tempo, a compensação gera maior \textit{overhead} de comunicação que é gasto no uso de comunicação/energia.
...
\begin{itemize}
 ...
 \item Alinhamento de \textit{Byte}: Diferentes tipos de alinhamento entre o transmissor e receptor, causam a necessidade de reordenar os \textit{bytes}. Alguns \textit{chips} de rádio não são capazes de resolver o alinhamento de um fluxo de mensagem, o receptor deverá resolver o \textit{offset} da mensagem.
\end{itemize}
