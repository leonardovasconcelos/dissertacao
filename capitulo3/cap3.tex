\chapter{Revisão detalhada do FTSP} \label{cap:cap3}
\outlineon=1

\outline{
\begin{itemize}
  \item Explicar a técnica.
  \item Inserir ilustração do processo de sincronização com o relógio externo. 
  \item Pseudo código
  \item FTSP+ e subsection
\end{itemize}
}


\begin{table}[htb!]
 \centering
 \begin{tabular}{c|c|c}	
 	\multirow{2}{*}{\textbf{Tempo}}	& \multirow{2}{*}{\textbf{Magnitude}}	& \multirow{2}{*}{\textbf{Distibuição}} \\
		&&\\
		\hline
      \multirow{2}{*}{ $\mathbf{Enviar~e~Receber}$ } 			&  \multirow{2}{*}{$0 - 100~ms$}	& Não determinístico,	\\
		&& depende da carga do CPU \\
		\hline
      \multirow{2}{*}{$\mathbf{Acesso~ao~Meio}$}  			&  \multirow{2}{*}{$10ms - 500~ms$}	& Não determinístico,	\\
		&& depende da contenção do canal \\
		\hline
      \multirow{2}{*}{$\mathbf{Transmissão/Recepção}$} 			& \multirow{2}{*}{$10ms - 20~ms$}		& Determinístico,	\\
		&& depende do tamanho da mensagem \\
		\hline
      \multirow{2}{*}{$\mathbf{Propagação}$} 				& $< 1\mu s$ para 			& Determinístico,	\\
		& distâncias acima de 300m & depende da distância\\
		\hline
      \multirow{2}{*}{$\mathbf{Tratar~Interrupção}$} 			& $< 5\mu s$ na maioria dos casos, 	& Não determinístico,	\\
		&mas pode chegar a $30\mu s$& depende das interrupções\\ 
		\hline
      \multirow{2}{*}{$\mathbf{Codificar/Decodificar}$} 		& $100\mu s - 200\mu s$,		& Determinístico,	\\
		&$< 2\mu s$ de variância& depende o chip de rádio \\
		\hline
      \multirow{2}{*}{$\mathbf{Alinhamento~de~Bytes}$} 			& \multirow{2}{*}{$0 - 400\mu s$} 	& Determinístico,	\\
		&& pode ser calculado
      
 \end{tabular}
 \caption{Fontes de atraso na transmissão de mensagens}
 \label{tab:mean_sd}
\end{table}
 
 

\section{\textit{Timestamp}}

\outline{
  Explicar como funciona a marcação de tempo no FTSP
}


\section{Escorregamento do Relógio}

\outline{
  Descrever os problemas relacionados ao clock drift.
  Como o FTSP corrige essa fonte de imprecisão.
}


\section{Sincronização Multi-saltos}

\outline{
\begin{itemize}
 \item Formato da mensagem de sincronização.
 \item Gerenciamento de informação redundante.
 \item Eleição do nó raiz.
 \item Evento: enviar/receber mensagem de sincronia. 
\end{itemize}

}